			...
			for (i=1;i<=9;i++)
			{
				...
				{ 
					...
					else
					{
						...
						mciSendString("open E:/Rings//fanhui.mp3 alias fhmusic3", NULL, 0, NULL);//是你自己有的 音乐文件所在地，盘， 路径，文件夹，文件名
...
void press()
{
	...
	putimage(-120, -35, &img_bk);	// 在坐标 (0, 0) 位置显示 IMAGE 对象
...
void gaming()
{
		...
		mciSendString("open E:/Rings//bkmusic.mp3 alias bkmusic", NULL, 0, NULL);//背景音乐是你自己有的 音乐文件所在地，盘， 路径，文件夹，文件名
...
void main()
{
	srand((unsigned)time(NULL));//根据时间变换随机产生
		starup();
		while(1)
		{
			switch(GamePrase)
